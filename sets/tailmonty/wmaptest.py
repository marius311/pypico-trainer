...
def get_pico():
    ...
    for region in ['inner7']:
        ...
        with open('reduced.%s.wmap.wmaplowell'%region) as f:
        ...
        work = [('wmap_lnl_lmax70', all_fit_inputter, DefaultOutputter(inputs,array([d['wmap_lnl_lmax70'] for d in samples]))),
                ('wmap_lnl_full', all_fit_inputter, DefaultOutputter(inputs,array([d['wmap_lnl_full'] for d in samples])))]
...
class tailmonty(PICO):
    ...
    def inputs(self):
        return set(region.inputter.inputs() for fit in self._data['fits'].values() for region in fit)
        
    def outputs(self):
        return self._data['fits'].keys()
